...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('addRowConfirmButton').addEventListener('click', addNewRow);
...
async function updateClass(event) {
    ...
    const cell = event.target.parentElement;
    const timeSlot = cell.parentElement.children[0].textContent.trim(); // Óra időpontja az első oszlopból
    const day = cell.parentElement.parentElement.parentElement.querySelector('caption').textContent.includes('Hétvégi') ? 
        ['Szombat', 'Vasárnap'][cell.cellIndex - 1] : 
        ['Hétfő', 'Kedd', 'Szerda', 'Csütörtök', 'Péntek'][cell.cellIndex - 1];

    const newClassName = prompt("Kérlek add meg az új órát:", cell.textContent.trim());
